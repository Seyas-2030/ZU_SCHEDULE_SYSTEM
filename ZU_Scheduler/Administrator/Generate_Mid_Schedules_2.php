...
if ($startDay === 'Friday' || $startDay === 'Saturday' || $endDay === 'Friday' || $endDay === 'Saturday') {
    echo "<script>alert('Start Date and End Date cannot be Friday or Saturday. Please select different dates.'); window.location.href='Generate_Mid_Schedules.php';</script>";
    exit();
}
...
if (empty($instructors)) {
    echo "<script>alert('No instructors available.'); window.location.href='Generate_Mid_Schedules.php';</script>";
    exit();
}

//Fetch instructor schedules to avoid conflicts
$queryInstructorSchedules = "SELECT * FROM instructors_mid_schedules";
...
//Fetch course mid hours in minutes
$queryCourses = "SELECT ID, Mid_Hours FROM courses WHERE Year = ? AND Semester = ? AND Department = ?";
...
while ($course = $resultCourses->fetch_assoc()) {
    $courses[$course['ID']] = $course['Mid_Hours'];
}
$resultCourses->free();
$stmtCourses->close();
...
function assignInstructor($scheduleItem, &$instructors, &$instructorSchedules, $dbConn) {
    foreach ($instructors as $instructor) {
        ...
        if ($isAvailable) {
            ...
            $query = "INSERT INTO instructors_mid_schedules (Instructor_ID, Schedule_ID) VALUES (?, ?)";
            $stmt = $dbConn->prepare($query);
...
foreach ($schedule as $item) {
    $checkQuery = "SELECT COUNT(*) FROM mid_schedules WHERE Section_ID = ? AND Date = ? AND Time_From = ? AND Time_To = ?";
    ...
    $queryInsert = "INSERT INTO mid_schedules (Section_ID, Room_ID, Date, Day, Time_From, Time_To, Year, Semester, Department) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    if (!assignInstructor($item, $instructors, $instructorSchedules, $dbConn)) {
        echo "<script>alert('Not enough instructors to cover all sections. Schedule generation aborted.'); window.location.href='Generate_Mid_Schedules.php';</script>";
        exit();
    }
}

echo "<script language='JavaScript'>
          alert('New Mid Schedules Has Been Generated Successfully!');
